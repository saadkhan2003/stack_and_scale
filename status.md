...
| 10    | OVH host provisioned; app pending  | Host SSH/Docker/UFW baseline, Cloudflare-only origin access and protected CI deployment secrets are configured. Rename the GitHub repository to lowercase, install the Cloudflare Origin Certificate and set Full (strict), generate server-local application secrets, deploy the application, verify the internal database has no public port, then rehearse deployment/rollback, browser edge behavior, capacity and independent-backup restoration. |
