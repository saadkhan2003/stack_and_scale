...
describe("CRM staff workflow", () => {
  ...
  beforeAll(async () => {
    ...
    await pool.query(
      "INSERT INTO platform.organizations (id, name) VALUES ($1, 'CRM Test')",
      [organizationId],
    );
    await pool.query(
      ...
      [
        managerId,
        memberId,
        `manager-crm-${suffix}@example.test`,
        `member-crm-${suffix}@example.test`,
      ],
    );
    await pool.query(
      "INSERT INTO identity.memberships (id, user_id, organization_id, role, status) VALUES ($1, $2, $3, 'manager', 'active'), ($4, $5, $3, 'member', 'active')",
      [
        `membership-manager-${suffix}`,
        managerId,
        organizationId,
        `membership-member-${suffix}`,
        memberId,
      ],
    );
    await pool.query(
  ...
  it("keeps the inbox closed to unauthenticated and member-only users", async () => {
    expect(
      (await fastify.inject({ method: "GET", url: "/api/v1/crm/leads" }))
        .statusCode,
    ).toBe(401);
    expect(
      (
  ...
  it("accepts a valid signed-in staff session without a development header", async () => {
    ...
    await pool.query(
      "INSERT INTO identity.sessions (id, user_id, status, mfa_satisfied, expires_at) VALUES ($1, $2, 'active', true, now() + interval '1 hour')",
      [sessionId, managerId],
    );
  ...
  it("lets a manager assign, progress, note, and complete a follow-up", async () => {
    ...
    const update = await fastify.inject({
      ...
      headers,
      payload: { ownerId: managerId, stage: "qualified", probability: 40 },
    });
    expect(update.statusCode).toBe(200);
    expect(
      update.json<{ data: { stage: string; ownerId: string } }>().data,
    ).toMatchObject({ stage: "qualified", ownerId: managerId });
    const note = await fastify.inject({
      method: "POST",
      url: `/api/v1/crm/leads/${leadId}/notes`,
      headers,
      payload: { body: "Called to discuss the demo scope." },
    });
    expect(note.statusCode).toBe(201);
    const task = await fastify.inject({
      method: "POST",
      url: `/api/v1/crm/leads/${leadId}/tasks`,
      headers,
      payload: { title: "Send follow-up", assigneeId: managerId },
    });
    expect(task.statusCode).toBe(201);
    const taskId = task.json<{ data: { id: string } }>().data.id;
    const complete = await fastify.inject({
      method: "PATCH",
      url: `/api/v1/crm/leads/${leadId}/tasks/${taskId}/complete`,
      headers: { "x-actor-id": managerId },
    });
