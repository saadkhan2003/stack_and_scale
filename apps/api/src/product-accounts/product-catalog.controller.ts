...
export class ProductCatalogController {
  ...
  public async flags(@Req() request: FastifyRequest, @Param("accountOrganizationId") accountOrganizationId: string, @Body() body: Record<string, unknown>) {
    const actor = await this.access.require(request, "org:manage"); return this.accounts.setAccountFlags(actor.actorId, accountOrganizationId, { ...(typeof body["accountEnabled"] === "boolean" ? { accountEnabled: body["accountEnabled"] } : {}), ...(typeof body["billingEnabled"] === "boolean" ? { billingEnabled: body["billingEnabled"] } : {}), ...(typeof body["downloadsEnabled"] === "boolean" ? { downloadsEnabled: body["downloadsEnabled"] } : {}), ...(typeof body["licenseEnforcementEnabled"] === "boolean" ? { licenseEnforcementEnabled: body["licenseEnforcementEnabled"] } : {}) });
  }
  @Post("licenses")
