...
export class ProductIntegrationAccessService {
  ...
  public async requireInstallation(request: FastifyRequest): Promise<ProductInstallationPrincipal> {
    ...
    if (!credential || credential.length < 32) throw new UnauthorizedException("Installation authentication is required.");
    ...
    const row = result.rows[0] as { installation_id: string; account_organization_id: string; product_id: string; license_id: string } | undefined;
    if (!row) throw new ForbiddenException("Installation access is not available.");
    return { installationId: row.installation_id, accountOrganizationId: row.account_organization_id, productId: row.product_id, licenseId: row.license_id };
  }
}
