...
export default async function ProductAccountPage(
  ...
  const account =
    typeof home["account"] === "object" && home["account"] !== null
      ? (home["account"] as Record<string, unknown>)
      : {};
  const accountName =
  ...
  return (
    <main className="site-shell" aria-labelledby="account-heading">
      ...
      <section>
        <h2>Subscriptions</h2>
        <ul>
          {values(home["subscriptions"]).map((item) => (
            <li key={String(item["id"])}>
              {String(item["status"])} — effective{" "}
      ...
      <section>
        <h2>Licenses</h2>
        <ul>
          {values(home["licenses"]).map((item) => (
            <li key={String(item["id"])}>
              {String(item["status"])} — {String(item["seat_limit"])} seats
            </li>
          ))}
        </ul>
      </section>
      <section>
        <h2>Billing</h2>
        <ul>
          {values(billing?.["invoices"]).map((item) => (
            <li key={String(item["id"])}>
              {String(item["status"])} — {String(item["currency"])}{" "}
              {String(item["amount_minor"])}
      ...
      <section>
        ...
        <ul>
          {values(home["releases"]).map((item) => (
            <li key={String(item["id"])}>
              {String(item["version"])} · {String(item["platform"])} · checksum
              verified
            </li>
          ))}
        </ul>
      ...
      <section>
        ...
        <ul>
          {values(support?.["support"]).map((item) => (
            <li key={String(item["id"])}>
      ...
      <section>
        <h2>Notifications</h2>
        <ul>
          {values(preferences?.["preferences"]).map((item) => (
            <li key={String(item["category"])}>
