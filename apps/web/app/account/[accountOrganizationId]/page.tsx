...
export default async function ProductAccountPage(props: Readonly<{ params: Promise<{ accountOrganizationId: string }> }>) {
  ...
  const [home, billing, support, preferences] = await Promise.all([getAccount(root, cookie), getAccount(`${root}/billing`, cookie), getAccount(`${root}/support`, cookie), getAccount(`${root}/notification-preferences`, cookie)]);
  ...
  return <main className="site-shell" aria-labelledby="account-heading">
    ...
    <section><h2>Billing</h2><ul>{values(billing?.["invoices"]).map((item) => <li key={String(item["id"])}>{String(item["status"])} — {String(item["currency"])} {String(item["amount_minor"])}{optionalText(item["payment_instruction"]) ? ` · ${optionalText(item["payment_instruction"])}` : ""}</li>)}</ul></section>
