...
export function LeadForm() {
  ...
  useEffect(() => { void fetch("/api/demo-slots").then((response) => response.ok ? response.json() as Promise<{ data?: unknown }> : null).then((payload) => setSlots(Array.isArray(payload?.data) ? payload.data.filter((slot): slot is string => typeof slot === "string") : [])).catch(() => setSlots([])); }, []);
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(await responseMessage(response, "We could not send your request."));
      ...
      if (body.intakeType === "demo") {
        ...
        if ((typeof startsAt === "string" && startsAt) || (typeof alternateRequest === "string" && alternateRequest.trim())) {
          const booking = await fetch(`/api/leads/${encodeURIComponent(receipt.id)}/bookings`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...(typeof startsAt === "string" && startsAt ? { startsAt: new Date(startsAt).toISOString() } : {}), timezone: Intl.DateTimeFormat().resolvedOptions().timeZone, ...(typeof alternateRequest === "string" && alternateRequest.trim() ? { alternateRequest } : {}) }) });
          if (!booking.ok) throw new Error(`Your enquiry was received, but ${await responseMessage(booking, "we could not reserve that demo time.")}`);
        }
      }
      ...
      setStatus("success"); event.currentTarget.reset(); setIntakeType("project");
  ...
  return <form className="lead-form" onSubmit={(event) => void submit(event)}><div className="form-grid"><label>Name<input autoComplete="name" name="name" required /></label><label>Email<input autoComplete="email" name="email" required type="email" /></label><label>Phone <span>(optional)</span><input autoComplete="tel" name="phone" type="tel" /></label><label>How can we help?<select name="intakeType" onChange={(event) => setIntakeType(event.target.value)} value={intakeType}><option value="demo">Book a product demo</option><option value="project">Discuss a custom project</option><option value="contact">General contact</option>{whatsappNumber ? <option value="whatsapp">Continue on WhatsApp</option> : null}</select></label></div>{intakeType === "demo" ? <fieldset className="demo-request"><legend>Book a product demo <span>(optional)</span></legend>{slots.length > 0 ? <label>Available time<select defaultValue="" name="demoStartsAt"><option value="">Choose a time</option>{slots.map((slot) => <option key={slot} value={slot}>{new Date(slot).toLocaleString()}</option>)}</select></label> : <p>No public slots are currently listed. Send an alternate time and our team will confirm it.</p>}<label>Alternative time or notes<textarea name="alternateRequest" rows={3} /></label></fieldset> : null}<label>What would you like to improve?<textarea name="message" rows={5} /></label><label className="honeypot" aria-hidden="true">Company<input autoComplete="off" name="company" tabIndex={-1} /></label><label className="consent-check"><input name="consent" required type="checkbox" /> <span>I agree that Stack &amp; Scale may use these details to respond to this request. <a href="/privacy">Privacy details</a>.</span></label>{status === "error" ? <p className="form-error" role="alert">{error} Your details are still in this form. You can try again or email hello@stackandscale.com.</p> : null}<button className="button button-primary" disabled={status === "sending"} type="submit">{status === "sending" ? "Sending…" : "Send request"}</button></form>;
}
