...
export function CmsPageLivePreview({
  initialPage,
}: Readonly<{ initialPage: CmsDocument }>) {
  ...
  useEffect(() => {
    ...
    const onMessage = (event: MessageEvent<unknown>) => {
      ...
      if (
        payload.type !== "payload-live-preview" ||
        payload.data === null ||
  ...
  return (
    <article className="detail-page">
      <p className="eyebrow">Published page</p>
      <h1>{page.title ?? "Untitled page"}</h1>
      <p className="detail-intro">
        {page.seo?.metaDescription ??
          "This page is published from the Stack & Scale CMS."}
      </p>
      <div className="detail-body">
        <CmsPageBlocks document={page} />
      </div>
    </article>
  );
}
