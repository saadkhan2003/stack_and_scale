...
V1 production runs on an owner-managed OVHcloud VPS-2 in London (UK), behind
Cloudflare. The Hetzner OpenTofu module remains a reference/disposable-staging
path and must not be applied to the OVH production host. Development, test,
preview, staging and production remain credential- and data-isolated.
Production origins accept only approved edge/reverse-proxy traffic where
practical. The self-hosted PostgreSQL, workers, CMS, identity and
storage-compatible services remain valid choices.
