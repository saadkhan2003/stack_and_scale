...
| :--- | :--- | :--- | :--- |
| **Muhammad Saad Khan** | Growth, Marketing & CMS Lead | `https://stackandscale.org`<br/>`https://cms.stackandscale.org/admin` | Publishing case studies, SEO articles, verifying lead intake forms and marketing funnels. |
| **Talha Shams** | Staff Operations & CRM Lead | `https://stackandscale.org/staff` | Single Sign-On (SSO), qualifying inbound leads, logging calls/tasks, creating commercial proposals and pricing tiers. |
| **Hanzala Khan** | Client Success & Storage Lead | `https://stackandscale.org/portal/...`<br/>`https://stackandscale.org/account/...` | Managing client onboarding, project milestone delivery, digital signature verification, and secure S3 file vaulting. |
| **Mehran Khan** | Platform Reliability & Security | `https://identity.stackandscale.org`<br/>`http://localhost:3001` (Grafana) / VPS SSH | Keycloak IAM administration, TLS edge protection, system metric audits, malware alert monitoring, and database backups. |
...
1. Navigate to the contact page: **[stackandscale.org/contact](https://stackandscale.org/contact)**
2. In the **Get in Touch** form, input the prospect's details:
...
4. **Behind the Scenes:**
   ...
   - The record is written to the `platform.leads` table with an initial status of `new`.
...
1. Open **[stackandscale.org/staff](https://stackandscale.org/staff)**.
2. If not already authenticated, the system redirects to Keycloak IAM (`https://identity.stackandscale.org`):
   - **Username:** `talhashams`
   - **Password:** `StackScale2026!#Talha`
3. Upon login, open the **Leads Pipeline** (`/staff/leads`).
4. **Reviewing the Lead:**
   - Click on the new lead card (`Sarah Jenkins — FinTech Global Systems`).
   - The **Lead 360 Drawer** opens on the right side of the screen.
5. **Updating Status & Adding Activity:**
   - Change **Status** from `new` &rarr; `contacted` &rarr; `qualified`.
...
2. Enter the commercial terms:
   - **Opportunity Title:** `FinTech Global - Cloud Infrastructure Modernization`
   - **Estimated Value:** `$45,000 USD`
   - **Target Close Date:** Select target closing date
3. Once converted, navigate to **[stackandscale.org/staff/proposals](https://stackandscale.org/staff/proposals)** and click **New Proposal**:
...
5. Click **Publish Proposal**. The system automatically provisions the organization ID and generates a secure portal link.

---

### Step 4: Client Portal Onboarding & Contract E-Signature (Hanzala Khan)

1. Navigate to the generated Client Portal URL:
...
3. **Digital Acceptance & Signature:**
...
4. The system updates the contract status to `Signed` and generates an invoice reference for accounting.
...
1. Inside the Client Portal, switch to the **Deliverables & Files** tab.
2. Click **Upload File** and select the document (e.g., `fintech-global-spec.pdf`).
3. **Behind the Scenes:**
