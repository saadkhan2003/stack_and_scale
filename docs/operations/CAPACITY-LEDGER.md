...
| Portals/control plane/integrations | 15–17         | Integration API: 100 sync mutations/1 MiB batch; one heartbeat/installation/5 min; event retries max 5; 30-day heartbeat and delivered-event retention                                                                  | Conflict and lease evidence retained; telemetry/delivered event records 30 days                              | Keep 30% host/database headroom; do not enable a customer until measured                            | Independently disable integration, telemetry, sync or delivery; throttle batches and pause consumers                                                | Sustained >70% CPU/memory/disk, database headroom <30%, event backlog >10,000 or heartbeat volume >12/installation/hour: separate app/database node, price only after provider approval                                             |
| Phase 18 evidence baseline         | 18.1          | One aggregate JSON record per protected capture: host load/memory/disk, running-service count, PostgreSQL connection count/database bytes, 24-hour API request/failure/mean-duration sums and outbox backlog age/counts | 90 days on the protected host; raw records are never committed                                               | Capture is negligible relative to host capacity and queries no customer records, labels or payloads | Disable/remove the collector; existing capacity view and services remain unchanged                                                                  | Any measured 70% host/database trigger, sustained queue/query latency breach, or user-demand bottleneck becomes an individually approved Phase 18 child proposal                                                                    |
