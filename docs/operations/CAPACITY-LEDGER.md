...
| Phase 18 evidence baseline         | 18.1          | One aggregate JSON record per protected manual capture: host load/memory/disk, running-service count, PostgreSQL connection count and database bytes                                        | 90 days on the protected host; raw records are never committed                                               | Capture is negligible relative to host capacity and does not query customer records          | Disable/remove the collector; existing capacity view and services remain unchanged                                                                  | Any measured 70% host/database trigger, sustained queue/query latency breach, or user-demand bottleneck becomes an individually approved Phase 18 child proposal                                                                    |
