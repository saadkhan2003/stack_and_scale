...
export class OfflineLeaseStore {
  ...
  public accept(lease: EntitlementLease, verificationKeys: VerificationKeys): void {
    ...
    if (Date.parse(lease.expiresAt) <= Date.parse(lease.issuedAt) || Date.parse(lease.graceUntil) < Date.parse(lease.expiresAt)) throw new Error("Lease time window is invalid.");
    this.lastSequence = lease.sequence; this.current = lease;
  }
...
export class EventDeduplicator {
  ...
  public accept(event: ProductIntegrationEvent, verificationKeys: VerificationKeys): boolean {
    ...
    this.ids.add(event.eventId); return true;
  }
}

export type IntegrationTransport = Readonly<{ request: (path: string, init?: Readonly<{ method?: "GET" | "POST"; body?: unknown; idempotencyKey?: string }>) => Promise<unknown> }>;

export class ProductIntegrationClient {
  public constructor(private readonly transport: IntegrationTransport, private readonly retryAttempts = 3) {}
  public async request(path: string, init?: Readonly<{ method?: "GET" | "POST"; body?: unknown; idempotencyKey?: string }>): Promise<unknown> {
    let lastError: unknown;
    for (let attempt = 0; attempt < this.retryAttempts; attempt += 1) {
      try { return await this.transport.request(path, init); }
