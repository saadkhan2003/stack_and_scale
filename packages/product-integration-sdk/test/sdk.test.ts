...
const publicKeys = {
  "key-1": pair.publicKey.export({ format: "pem", type: "spki" }).toString(),
};
function lease(sequence: number): EntitlementLease {
  const unsigned = {
...
describe("product integration SDK", () => {
  it("verifies leases, enforces monotonic sequence, and uses bounded grace", () => {
    const store = new OfflineLeaseStore();
    store.accept(lease(2), publicKeys);
    ...
    expect(() =>
      new OfflineLeaseStore().accept(lease(3), {
        "key-1": { publicKey: publicKeys["key-1"], status: "revoked" },
      }),
    ).toThrow("revoked");
  });
  ...
  it("verifies and deduplicates signed unordered events", () => {
    const unsigned = {
      contractVersion: PRODUCT_INTEGRATION_CONTRACT_VERSION,
      eventId: "event-1",
      type: "entitlement.lease_issued",
      source: "platform" as const,
      subject: { kind: "installation" as const, id: "installation-1" },
      occurredAt: "2026-09-01T00:00:00.000Z",
      payloadVersion: 1 as const,
      payload: {},
      keyId: "key-1",
    };
    const event: ProductIntegrationEvent = {
      ...unsigned,
      signature: sign(
        null,
        Buffer.from(canonicalProductIntegrationJson(unsigned)),
    ...
    expect(deduplicator.accept(event, publicKeys)).toBe(true);
    expect(deduplicator.accept(event, publicKeys)).toBe(false);
    const reordered: ProductIntegrationEvent = {
      ...event,
      eventId: "event-0",
      signature: sign(
        null,
        Buffer.from(
          canonicalProductIntegrationJson({ ...unsigned, eventId: "event-0" }),
        ),
        pair.privateKey,
      ).toString("base64url"),
    };
  ...
  it("retries transport failures without embedding credentials", async () => {
    ...
    await expect(client.request("/lease", { method: "POST" })).resolves.toEqual(
      { ok: true },
    );
    expect(calls).toBe(3);
  });
